...
df = pd.read_csv("./login.csv", sep=";", header=0, parse_dates=[1], index_col=[1])
...
test_data = tsr_rol[75:]

tsr_sea_dec = seasonal_decompose(train_data, model='add', period=8)

tsr_sea_dec.plot()

tsmodel = ExponentialSmoothing(train_data, trend='add', seasonal='add', seasonal_periods=8).fit()
prediction = tsmodel.forecast(4)
# Original Series
...
# Future Series
tsmodel = ExponentialSmoothing(tsr_rol, trend='add', seasonal='add', seasonal_periods=8).fit()
prediction = tsmodel.forecast(4)
plt.figure(figsize=(24,10), dpi=100)
plt.ylabel('Values',fontsize=14)
plt.xlabel('Time',fontsize=14)
